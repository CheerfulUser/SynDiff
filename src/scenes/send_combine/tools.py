...
def find_unique2ps1(ps1_cat, viz_cat, catalog):
    ...
    # print('UNIQUE PS1', len(viz_cat), len(ps1_cat))
    try:
        idx, d2d, d3d = coords_viz.match_to_catalog_3d(coords_obs)
    except:
        print('Error matching PS1 catalog to Vizier catalog')
        # print('ZZZ Catalog', catalog)
        # print('ZZZ PS1:', ps1_cat, type(ps1_cat))
        # print('ZZZ PS1 RA:', ps1_cat.raMean, type(ps1_cat.raMean))
        # print('ZZZ PS1 Dec:', ps1_cat.decMean, type(ps1_cat.decMean))
        return None
