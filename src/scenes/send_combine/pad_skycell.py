...
class pad_skycell():
    def __init__(self,ps1,skycells,datapath,pad=500,
        ...
        self.padded_skycell = deepcopy(self.ps1.padded)
        
        # print('ZZZ Initial Load', self.padded_skycell.shape)
    ...
    def run(self):
        # print('ZZZ Before Oversize', self.padded_skycell.shape)
        self._ovsersize_points()
        # print('ZZZ After Oversize', self.padded_skycell.shape)
        self._check_skycells()
        # print('ZZZ After check skycells', self.padded_skycell.shape)
        self._skycell_poly()
        # print('ZZZ After skycell poly', self.padded_skycell.shape)
        self._intersecting_points()
        # print('ZZZ After intersecting points', self.padded_skycell.shape)
        self._pad_sides()
        # print('ZZZ After pad sides', self.padded_skycell.shape)
        self._pad_corners()
        # print('ZZZ After pad corners', self.padded_skycell.shape)
        # print('FILLING CATALOG')
        self._cat_fill()
        self.padded = deepcopy(self.padded_skycell)
        self.ps1.padded = deepcopy(self.padded_skycell)
        # print(f'PS1 Padded {self.ps1.padded.shape}')
        # print('Mask Shape', self.ps1.mask.shape)
    ...
    def _pad_sides(self):
        ...
        # print('ZZZ SIDES')
        temp_padded = deepcopy(self.padded_skycell)
        # print('ZZZ Temp', temp_padded.shape)
        ...
        for i in range(len(side_cells[0])):
            ...
            # print(f'Padding {name} side {side} with {pad} pixels')
            # print(buff.shape)
            # print(self.padded_skycell.shape)
            # print(overlap)
            # raise ValueError('Testing')

            if side == 0:
                temp_padded[pad:-pad,:pad+10] = buff[:,-(pad+overlap):-overlap+10]
            elif side == 1:
                temp_padded[-(pad+10):,pad:-pad] = buff[overlap-10:(pad+overlap),:]
            elif side == 2:
                temp_padded[pad:-pad,-(pad+10):] = buff[:,overlap-10:(pad+overlap)]
            elif side == 3:
                temp_padded[:pad+10,pad:-pad] = buff[-(pad+overlap):-overlap+10,:]
            
        self.padded_skycell = deepcopy(temp_padded)
        # print('ZZZ Padded', self.padded_skycell.shape)
    ...
    def _pad_corners(self):
        ...
        temp_padded = deepcopy(self.padded_skycell)
        # print('ZZZ Temp Corner', temp_padded.shape)
        for i in range(len(corner_cells[0])):
            ...
            buff = hdul[j].data
            if corner == 0:
                temp_padded[:(pad+10),:(pad+10)] = buff[-(pad+overlap):-overlap+10,-(pad+overlap):-overlap+10]
            elif corner == 1:
                temp_padded[-(pad+10):,:(pad+10)] = buff[overlap-10:(pad+overlap),-(pad+overlap):-overlap+10]
            elif corner == 2:
                temp_padded[-(pad+10):,-(pad+10):] = buff[overlap-10:(pad+overlap),overlap-10:(pad+overlap)]
            elif corner == 3:
                temp_padded[:pad+10,-pad-10:] = buff[-(pad+overlap):-overlap+10,overlap-10:(pad+overlap)]
        # print('ZZZ Temp Corner', temp_padded.shape)
        self.padded_skycell = deepcopy(temp_padded)
        # print('ZZZ Padded Corner', self.padded_skycell.shape)
    ...
    def _cat_fill(self):
        ...
        if (len(sides) > 0) & (len(corners) > 0):
            if self.ps1.cat is not None:
                ...
                # print(f'PS1 Padded {self.padded_skycell.shape}')
                ind = (x > 0) & (y > 0) & (x < self.padded_skycell.shape[1]) & (y < self.padded_skycell.shape[0])
                cat = cat.iloc[ind]
                cat_image = np.zeros_like(self.padded_skycell)
                ...
                self.padded_skycell[ind_image > 0] = cat_image[ind_image > 0] + self.ps1.image_stats[1]
            else:
                print('No catalog provided!')
            
    def _catpad_index(self,sides,corners):
        pad = self.pad
        catpad = np.zeros_like(self.padded_skycell)
        for side in sides:
            if side == 0:
    ...
    def plot_image(self):
        ...
        vmin = np.nanpercentile(self.padded_skycell,16)
        vmax = np.nanpercentile(self.padded_skycell,90)
        plt.figure()
        plt.imshow(self.padded_skycell,vmin=vmin,vmax=vmax,origin='lower')
        codes = [
                Path.MOVETO,
                Path.LINETO,
                Path.LINETO,
