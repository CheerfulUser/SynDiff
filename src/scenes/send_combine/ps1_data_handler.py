...
class ps1_data():
    ...
    def _load_image(self,file=None):
        ...
        self.exptime = deepcopy(self.header['EXPTIME'])
    ...
    def _load_mask(self,mask):
        if mask:
            ...
            self.mask = np.pad(hdul[j].data,self.pad) / self.exptime
    ...
    def convert_flux_scale(self,toflux=True):
        ...
        if toflux:
            ...
            self.data = flux / self.exptime

            x = self.padded/a
            flux = self.header['BOFFSET'] + self.header['BSOFTEN'] * 2 * np.sinh(x)
            self.padded = flux / self.exptime
                
                # print(f'ZZZ {self.padded}')
                # print(f'ZZZ {self.padded.shape}')
        else:
            if self.ftype == 'flux':
                self.ftype = 'log'
